...
## Phase 9 cold-start routing

The serving policy is explicit:

| Situation | Recommendation source |
| --- | --- |
| Known user with personalized candidates | Validation-selected CF-only hybrid |
| Anonymous or unknown user | Global popularity ranking |
| Known user with no personalized candidates | Global popularity ranking |

The router validates positive user IDs, limits, and seen-item input. It always filters seen items. It also returns a machine-readable route label (`personalized_cf` or `popularity_cold_start`), allowing the future API/UI to state why a result is not personalized.

On the existing held-out personalized evaluation, all 2,275 eligible users received the `personalized_cf` route and none fell back; precision@10 0.066110, recall@10 0.549275, and NDCG@10 0.354436 are therefore unchanged. Executed simulated cold-start checks confirmed that anonymous and unknown users each receive 10 popularity items and that the fallback respects seen-item filtering.

No cold-start relevance metric is reported. A genuinely new user has no subsequent interactions in this frozen dataset, so assigning relevance or using a proxy would fabricate an evaluation target. This limitation is intentional and must be stated in the final presentation.

Executed details are saved in Git-ignored `data/processed/cold_start_evaluation.json` and are generated by `src.cold_start`.

## Remaining evaluation work

The next evaluation work belongs to API-level integration and end-to-end tests; do not choose model parameters using test results.
